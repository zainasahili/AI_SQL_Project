...
<pre> ``` sql SELECT strftime('%H', departure_time) AS hour, COUNT(*) AS num_departures FROM flight GROUP BY hour ORDER BY num_departures DESC LIMIT 1; ```</pre>
...
<pre> ``` sql SELECT a.airport_name, COUNT(f.id) AS departure_count \nFROM airport a \nJOIN flight f ON a.id = f.departure_airport_id \nGROUP BY a.id \nORDER BY departure_count DESC \nLIMIT 1; ```</pre> 
